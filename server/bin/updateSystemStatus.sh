...
# start storescp
if [[ "$v1" == "0" ]]; then
   su - processing -c "${SERVERDIR}/bin/storectl.sh stop; sleep 1; /usr/bin/python3 ${SERVERDIR}/bin/processSingleFile3.py stop"
   ...
   su - processing -c "/usr/bin/python3 ${SERVERDIR}/bin/processSingleFile3.py start; sleep 1; ${SERVERDIR}/bin/storectl.sh start &"
