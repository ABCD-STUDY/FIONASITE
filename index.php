...
function loadSubjects() {
	...
	for (var i = 0; i < data.length; i++) {
            var shortname = data[i].PatientName;
            if (shortname.length > l)
               shortname = data[i].PatientName.substring(0, l) + "...";

	    jQuery('#list-of-subjects').prepend('<div class="data open-study-info" style="position: relative;" studyinstanceuid="'+data[i].StudyInstanceUID+'"><a class="mdl-navigation__link" href="#" title=\"' + data[i].PatientName + '\"><i class="mdl-color-text--blue-grey-400 material-icons" role="presentation">accessibility</i><div style="font-size: 10px; position: absolute; left: 55px; bottom: -2px;">' + data[i].StudyDate + '/' + data[i].StudyTime.split('.')[0] + '</div>'+shortname+'</a></div>');
	}
    });
}
function shortenName( name ) {
   ...
   if (name !== null && name.length > l) {
       // take the first part
       return name.substring(0,5) + "..." + name.substring(name.length -15, name.length);
   }
   return name;
}
var studyData = [];
function loadStudies() {
	...
	str = "<ul id=\"study-list-bonsai\" class=\"data\">";
	   ...
	   str = str + "<li title=\""+data[studies[i]][0]['PatientName']+"\">" + data[studies[i]][0]['PatientID'] + "<ul>";
...
function loadScanner() {
	...
	for (var i = 0; i < data.length; i++) {
	      ...
	      data[i].PatientName = data[i].PatientID;
           }
	   str = str + "<li><span title=\"" + data[i].PatientID + "\">" + data[i].PatientName + "</span><br/><small style=\"padding-top: -10px;\">"+data[i].StudyDate+" "+data[i].StudyTime+"</small>&nbsp;&nbsp;<button style=\"margin-top: -20px;\" class=\"mdl-button mdl-js-button mdl-button--icon pull-study\" study=\""+data[i].StudyInstanceUID+"\" title=\"Downlaod to FIONA\"><i class=\"material-icons\">touch_app</i></button><ul>";
